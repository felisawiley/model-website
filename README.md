...
6. Your site will be live at `https://felisawiley.com` (custom domain via `CNAME`) or `https://YOUR_USERNAME.github.io/fee-portfolio/` within a few minutes.
...
- **SEO / live URL:** Domain is set to `felisawiley.com` in `CNAME`, `js/config.js`, `robots.txt`, `sitemap.xml`, and page meta tags. Pages use clean URLs (no `.html`); each route is a folder with an `index.html` inside.
